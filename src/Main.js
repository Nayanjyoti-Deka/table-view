...
export default class Main extends Component {
    ...
    renderTableData() {
        return this.state.users.map((student, index) => {
            if (index < page) {
                return (
                    <tr key={index} onClick={() => this.detailView(student)} style={{cursor:'pointer'}}>
    ...
    renderTableHeader() {
        ...
        return filtered.map((eachHeader, index) => {
            ...
            return <td><img style={{cursor:'pointer'}} onClick={() => this.sort(header, eachHeader)} src={require('' + image)} height="15"
    ...
    pagination() {
        ...
        let newPageArr = [];
        // if(this.state.currentPagrNo>10){
        //   for (let i = 0; i < noOfPage; i++) {
        //       pageArr.push(i+this.state.currentPagrNo);
        //       // if (i < buttonPerPage) {
        //       //     newPageArr.push(i);
        //       // }
        //
        ...
        return pageArr.map((value, index) => {
            if (index == 0) {
                return (<span>
                    <button style={{cursor:'pointer'}} onClick={() => this.goToPage(this.state.currentPagrNo - 1, page)}>  &lt; &lt;Previous</button>
                    <button style={{cursor:'pointer'}} onClick={() => this.goToPage(value + 1, page, noOfPage)}>{value + 1}</button>
                    </span>)
            }
            if (index < buttonPerPage) {
                return (<button style={{cursor:'pointer'}} onClick={() => this.goToPage(value + 1, page, noOfPage)}>{value + 1}</button>)
            }
            if (index === buttonPerPage) {
                return (<button style={{cursor:'pointer'}} onClick={() => this.goToPage(this.state.currentPagrNo + 1, page, noOfPage)}>Next>></button>)
            }
        })
    }
    ...
    goToPage(pNo, pageSize, noOfPage) {
        ...
        this.setState({currentPagrNo: pNo,users: arr,startIndex:start});
